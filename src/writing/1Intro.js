const Intro = () => {
  return (
    <div>
      <h1 className="text-3xl sm:text-5xl font-bold mb-8 text-center text-gray-900 mt-8">
        Build Tokenizer From Scratch - Part 1
      </h1>
      <div className="max-w-xl mx-auto">
        <p>
          Esther began learning how to build the GPT tokenizer with Andrej
          Karpathy. As she was learning, she realized, "OMG! The content is
          AMAZING! But why is it so... inhuman?" (Youtube content greater than
          30min = inhuman). The video is a scary 2 hours. Most of the time,
          people don’t even dare to start. Can we make the learning less scary
          and more fun? <br></br>hi, future Esther (or some other humans), let
          me teach you what is tokenization.
        </p>
        <p class="italic text-center text-gray-600 bg-gray-200 p-2 rounded-lg shadow-md">
          For tiny tiny phone human beings: Flip your screen horizontally to
          save my life.
        </p>
      </div>
    </div>
  );
};

export default Intro;
